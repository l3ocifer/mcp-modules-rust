...
impl<'a> SteamClient<'a> {
    ...
    pub async fn list_friends(&self, steam_id: Option<&str>) -> Result<Vec<SteamFriend>> {
        ...
        let friend_list_data: Value = friend_list_response
            .json()
            .await
            .map_err(|e| Error::protocol(format!("Failed to parse friend list response: {}", e)))?;

        // Extract friend steam IDs
        let friend_ids = match friend_list_data
            .get("friendslist")
            .and_then(|f| f.get("friends"))
        {
            Some(friends) => match friends.as_array() {
                Some(friends_array) => friends_array
                    .iter()
                    .filter_map(|f| {
                        f.get("steamid")
                ...
                None => {
                    return Err(Error::protocol(
                        "Invalid friends array in response".to_string(),
                    ))
                }
            },
        ...
        let summaries_data: Value = summaries_response.json().await.map_err(|e| {
            Error::protocol(format!("Failed to parse player summaries response: {}", e))
        })?;

        // Extract player information
        let players = match summaries_data
            .get("response")
            .and_then(|r| r.get("players"))
        {
            Some(players) => match players.as_array() {
                Some(players_array) => players_array
                    .iter()
                    .filter_map(|p| {
                ...
                None => {
                    return Err(Error::protocol(
                        "Invalid players array in response".to_string(),
                    ))
                }
            },
            None => {
                return Err(Error::protocol(
                    "Failed to get player information".to_string(),
                ))
            }
        };
    ...
    pub async fn list_games(&self, steam_id: Option<&str>) -> Result<Vec<SteamGame>> {
        ...
        let games_data: Value = games_response
            .json()
            .await
        ...
        let games = match games_data.get("response").and_then(|r| r.get("games")) {
            Some(games) => {
                match games.as_array() {
                    ...
                    None => {
                        return Err(Error::protocol(
                            "Invalid games array in response".to_string(),
                        ))
                    }
                }
            }
            None => {
                return Err(Error::protocol(
                    "Failed to get games information".to_string(),
                ))
            }
        };
    ...
    pub async fn get_player_summary(&self, steam_id: Option<&str>) -> Result<SteamFriend> {
        ...
        let summary_data: Value = summary_response.json().await.map_err(|e| {
            Error::protocol(format!("Failed to parse player summary response: {}", e))
        })?;
            ...
            Some(players) => match players.as_array() {
                Some(players_array) if !players_array.is_empty() => {
                    ...
                    let steam_id = p
                        ...
                        .and_then(|id| id.as_str())
                        .ok_or_else(|| {
                            Error::protocol("Failed to get player steam ID".to_string())
                        })?
                        .to_string();
