...
use crate::cloud::{
    AwsConfig, CloudProvider, CloudResource, ComplexityLevel, CostOptimization, CostRecommendation,
    PaymentOption, RecommendationPriority, ReservedInstanceRecommendation, ReservedInstanceTerm,
    RightsizingRecommendation, SecurityAssessment, SecurityRecommendation, SecurityViolation,
    ViolationSeverity,
};
...
impl AwsClient {
            ...
            for bucket in buckets {
                ...
                let security_score = if bucket.encryption.is_some()
                    && bucket
                        .public_access_block
                        .as_ref()
                        .is_some_and(|pab| pab.block_public_acls)
                {
                    85.0
                } else {
                    60.0
                        ...
                        if let Some(instance_array) = instance_data.as_array() {
                            if instance_array.len() >= 11 {
                                ...
                                if let Some(tags_array) =
                                    instance_array.get(11).and_then(|t| t.as_array())
                                {
                                ...
                                instances.push(Ec2Instance {
                                    ...
                                    security_groups: vec![instance_array[5]
                                        .as_str()
                                        .unwrap_or("")
                                        .to_string()],
                                    public_ip: instance_array[6].as_str().map(|s| s.to_string()),
                                    private_ip: instance_array[7]
                                        .as_str()
                                        .unwrap_or("")
                                        .to_string(),
                                    launch_time: instance_array[8]
                                        .as_str()
                                        .unwrap_or("")
                                        .to_string(),
                                    platform: instance_array[9].as_str().map(|s| s.to_string()),
                                    architecture: instance_array[10]
                                        .as_str()
                                        .unwrap_or("x86_64")
                                        .to_string(),
    ...
    pub async fn list_lambda_functions(&self) -> Result<Vec<LambdaFunction>> {
        let output = self
            .execute_aws_command(&["lambda", "list-functions", "--output", "json"])
            .await?;
        ...
        if let Some(functions_array) = functions_data.get("Functions").and_then(|f| f.as_array()) {
            for function_data in functions_array {
                ...
                functions.push(LambdaFunction {
                    ...
                    reserved_concurrency: config_data
                        .get("ReservedConcurrencyExecutions")
                        .and_then(|c| c.as_i64())
                        .map(|c| c as i32),
    ...
    pub async fn list_s3_buckets(&self) -> Result<Vec<S3Bucket>> {
        let output = self
            .execute_aws_command(&["s3api", "list-buckets", "--output", "json"])
            .await?;
    ...
    pub async fn cost_optimization(&self) -> Result<CostOptimization> {
        ...
        recommendations.push(CostRecommendation {
            ...
            description: "Enable detailed billing to track costs by service and resource"
                .to_string(),
        ...
        recommendations.push(CostRecommendation {
            ...
            potential_savings: 0.0,
            description: "Create budget alerts to monitor spending and prevent cost overruns"
                .to_string(),
            complexity: ComplexityLevel::Low,
        });
