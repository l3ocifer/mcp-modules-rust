...
impl ContainerClient {
    ...
    pub async fn new(lifecycle: Arc<LifecycleManager>) -> Result<Self> {
        ...
        for runtime in &[ContainerRuntime::Podman, ContainerRuntime::Docker, ContainerRuntime::Containerd] {
    ...
    async fn run_runtime_command(&self, runtime: &ContainerRuntime, args: &[&str]) -> Result<String> {
        let output = Command::new(runtime.as_str())
            .args(args)
            .output()
            .await
            .map_err(|e| Error::internal(format!("Failed to execute {} command: {}", runtime.as_str(), e)))?;
        
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(Error::internal(format!("{} command failed: {}", runtime.as_str(), stderr)));
        }
    ...
    pub async fn list_containers(&self, runtime: Option<ContainerRuntime>, show_all: bool) -> Result<Vec<Container>> {
        ...
        for line in output.lines().skip(1) { // Skip header
            ...
            if parts.len() >= 6 {
                let ports = self.parse_ports(parts.get(5).unwrap_or(&"")).unwrap_or_default();
        ...
        for port_mapping in ports_str.split(',') {
            ...
            if let Some(arrow_pos) = port_mapping.find("->") {
                ...
                let (container_port, protocol) = if let Some(slash_pos) = container_part.find('/') {
                    ...
                    (container_part.parse::<u16>().unwrap_or(0), "tcp".to_string())
                };
    ...
    pub async fn create_container(&self, params: ContainerCreateParams) -> Result<String> {
        ...
        self.security.validate_input(&params.image, &crate::security::SanitizationOptions {
            max_length: Some(256),
            allow_html: false,
            allow_sql: false,
        ...
        for port in &params.ports {
            let port_str = if let Some(ref host_ip) = port.host_ip {
                format!("{}:{}:{}/{}", host_ip, port.host_port, port.container_port, port.protocol)
            } else {
        ...
        if let Some(ref security) = params.security_context {
            ...
            if let Some(ref profile) = security.apparmor_profile {
                security_strings.push(("--security-opt".to_string(), format!("apparmor={}", profile)));
            }
        }
        ...
        if let Some(ref health) = params.health_check {
            ...
            health_strings.push(("--health-interval".to_string(), format!("{}s", health.interval_seconds)));
            health_strings.push(("--health-timeout".to_string(), format!("{}s", health.timeout_seconds)));
        ...
        if params.rootless && runtime == &ContainerRuntime::Docker {
            ...
            self.security.log_security_event("ROOTLESS_REQUEST", Some("Rootless requested for Docker"));
        }
        ...
        let output = self.run_runtime_command(runtime, &args).await?;
        self.security.log_security_event("CONTAINER_CREATED", Some(&format!("Runtime: {}, Image: {}", runtime.as_str(), params.image)));
    ...
    pub async fn create_pod(&self, config: PodConfig) -> Result<String> {
        ...
        let output = self.run_runtime_command(&ContainerRuntime::Podman, &args).await?;
        self.security.log_security_event("POD_CREATED", Some(&config.name));
        
        Ok(output.trim().to_string())
    }

    /// Get container statistics
    pub async fn get_container_stats(&self, container_id: &str, runtime: Option<ContainerRuntime>) -> Result<ResourceUsage> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        let output = self.run_runtime_command(&runtime, &["stats", "--no-stream", "--format", "json", container_id]).await?;
        ...
        Ok(ResourceUsage {
            cpu_percent: stats.get("CPUPerc").and_then(|v| v.as_str())
                .and_then(|s| s.trim_end_matches('%').parse().ok()).unwrap_or(0.0),
            memory_usage: stats.get("MemUsage").and_then(|v| v.as_str())
                .and_then(|s| s.split('/').next())
                .and_then(|s| s.trim().parse().ok()).unwrap_or(0),
            memory_limit: stats.get("MemUsage").and_then(|v| v.as_str())
                .and_then(|s| s.split('/').nth(1))
                .and_then(|s| s.trim().parse().ok()).unwrap_or(0),
            network_io: NetworkIO { rx_bytes: 0, tx_bytes: 0 }, // Would need detailed parsing
            block_io: BlockIO { read_bytes: 0, write_bytes: 0 }, // Would need detailed parsing
        })
    }

    /// Stop a container
    pub async fn stop_container(&self, id: &str, runtime: Option<ContainerRuntime>) -> Result<String> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        let output = self.run_runtime_command(&runtime, &["stop", id]).await?;
        self.security.log_security_event("CONTAINER_STOPPED", Some(id));
        Ok(output)
    }

    /// Start a container
    pub async fn start_container(&self, id: &str, runtime: Option<ContainerRuntime>) -> Result<String> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        let output = self.run_runtime_command(&runtime, &["start", id]).await?;
        self.security.log_security_event("CONTAINER_STARTED", Some(id));
        Ok(output)
    }

    /// Remove a container
    pub async fn remove_container(&self, id: &str, force: bool, runtime: Option<ContainerRuntime>) -> Result<String> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        let mut args = vec!["rm"];
        if force {
            args.push("-f");
        ...
        let output = self.run_runtime_command(&runtime, &args).await?;
        self.security.log_security_event("CONTAINER_REMOVED", Some(id));
        Ok(output)
    }

    /// Get container logs with enhanced options
    pub async fn get_container_logs(&self, id: &str, tail: Option<u32>, follow: bool, timestamps: bool, runtime: Option<ContainerRuntime>) -> Result<String> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
    ...
    pub async fn exec_in_container(&self, id: &str, command: &[&str], interactive: bool, runtime: Option<ContainerRuntime>) -> Result<String> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        ...
        self.security.log_security_event("CONTAINER_EXEC", Some(&format!("{}: {}", id, command.join(" "))));
        self.run_runtime_command(&runtime, &args).await
    }

    /// Check security posture of a container
    pub async fn security_scan(&self, container_id: &str, runtime: Option<ContainerRuntime>) -> Result<SecurityScanResult> {
        let runtime = runtime.unwrap_or(self.default_runtime.clone());
        
        // Get container inspect information
        let inspect_output = self.run_runtime_command(&runtime, &["inspect", container_id]).await?;
