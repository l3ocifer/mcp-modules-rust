...
impl ServerManager {
    ...
    pub async fn new() -> Result<Self> {
        ...
        #[cfg(unix)]
        {
            ...
            let mut perms = fs::metadata(&base_dir)
                .map_err(|e| {
                    Error::internal(format!("Failed to get directory permissions: {}", e))
                })?
                .permissions();
            perms.set_mode(0o700); // Only owner can read/write/execute
            fs::set_permissions(&base_dir, perms).map_err(|e| {
                Error::internal(format!("Failed to set directory permissions: {}", e))
            })?;
        }

        security.log_security_event(
            "SERVER_MANAGER_CREATED",
    ...
    fn validate_server_code(
        &self,
        code: &str,
        language: &crate::creation::ServerLanguage,
    ) -> Result<()> {
        // Basic validation for malicious patterns
        let dangerous_patterns = match language {
            crate::creation::ServerLanguage::TypeScript
            | crate::creation::ServerLanguage::JavaScript => vec![
        ...
        match self.security.validate_input(code, &validation_opts) {
            ...
            crate::security::ValidationResult::Invalid(reason)
            | crate::security::ValidationResult::Malicious(reason) => {
                self.security
                    .log_security_event("MALICIOUS_CODE_DETECTED", Some(&reason));
        ...
        for pattern in dangerous_patterns {
            if let Ok(regex) = regex::Regex::new(pattern) {
                if regex.is_match(code) {
                    self.security
                        .log_security_event("DANGEROUS_PATTERN_DETECTED", Some(pattern));
                    return Err(Error::validation(format!(
    ...
    pub fn validate_javascript_code(&self, code: &str) -> Result<()> {
        ...
        for pattern in &dangerous_patterns {
            if let Ok(regex) = regex::Regex::new(pattern) {
                if regex.is_match(code) {
                    self.security
                        .log_security_event("DANGEROUS_JS_PATTERN", Some(pattern));
                    return Err(Error::validation(format!(
                        "Dangerous JavaScript pattern detected: {}",
                        pattern
                    )));
                }
            }
    ...
    pub fn validate_python_code(&self, code: &str) -> Result<()> {
        ...
        for pattern in &dangerous_patterns {
            if let Ok(regex) = regex::Regex::new(pattern) {
                if regex.is_match(code) {
                    self.security
                        .log_security_event("DANGEROUS_PYTHON_PATTERN", Some(pattern));
                    return Err(Error::validation(format!(
    ...
    pub fn validate_rust_code(&self, code: &str) -> Result<()> {
        ...
        for pattern in &dangerous_patterns {
            if let Ok(regex) = regex::Regex::new(pattern) {
                if regex.is_match(code) {
                    self.security
                        .log_security_event("DANGEROUS_RUST_PATTERN", Some(pattern));
                    return Err(Error::validation(format!(
                        "Dangerous Rust pattern detected: {}",
                        pattern
                    )));
                }
            }
    ...
    fn check_server_limit(&self) -> Result<()> {
        ...
        if servers.len() >= self.max_servers {
            self.security
                .log_security_event("SERVER_LIMIT_EXCEEDED", Some(&self.max_servers.to_string()));
            return Err(Error::validation(format!(
    ...
    pub async fn create_server(
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&server_dir, perms).map_err(|e| {
                Error::internal(format!("Failed to set directory permissions: {}", e))
            })?;
        }
        ...
        let child = self
            .start_secure_server_process(&language, file_path.as_path(), server_dir.as_path())
            .await?;
        ...
        self.security
            .log_security_event("SERVER_CREATED", Some(&server_id));
        Ok(server_id)
    }

    /// Start server process with security restrictions
    async fn start_secure_server_process(
        &self,
        language: &crate::creation::ServerLanguage,
        file_path: &Path,
        server_dir: &Path,
        ...
        match self.security.validate_input(command, &validation_opts) {
            ...
            | crate::security::ValidationResult::Malicious(reason) => {
                self.security
                    .log_security_event("MALICIOUS_SERVER_COMMAND", Some(&reason));
                return Err(Error::validation(format!(
        ...
        for arg in &args {
            match self.security.validate_input(arg, &validation_opts) {
                ...
                | crate::security::ValidationResult::Malicious(reason) => {
                    self.security
                        .log_security_event("MALICIOUS_SERVER_ARG", Some(&reason));
                    return Err(Error::validation(format!(
        ...
        self.security
            .log_security_event("SERVER_PROCESS_STARTED", Some(command));
        Ok(child)
    }
    ...
    pub async fn delete_server(&self, server_id: &str) -> Result<()> {
        ...
        match self.security.validate_input(server_id, &validation_opts) {
            ...
            | crate::security::ValidationResult::Malicious(reason) => {
                self.security
                    .log_security_event("INVALID_SERVER_ID", Some(&reason));
        ...
        if let Err(e) =
            tokio::time::timeout(Duration::from_secs(10), server_process.process.kill()).await
        {
            self.security.log_security_event(
                "SERVER_KILL_TIMEOUT",
                Some(&format!("Server: {}, Error: {}", server_id, e)),
            );
        }
        ...
        self.security
            .log_security_event("SERVER_DELETED", Some(server_id));
        Ok(())
    }
    ...
    pub async fn update_server(&self, server_id: &str, code: &str) -> Result<String> {
        ...
        self.security.log_security_event(
            "SERVER_UPDATED",
            Some(&format!("Old: {}, New: {}", server_id, new_server_id)),
        );
        Ok(new_server_id)
    }
    ...
    pub async fn get_server_client(&self, server_id: &str) -> Result<Arc<LifecycleManager>> {
        ...
        let transport_box =
            Box::new(transport) as Box<dyn crate::transport::Transport + Send + Sync>;
    ...
    pub async fn execute_tool(
        ...
        match self.security.validate_input(tool_name, &validation_opts) {
            ...
            | crate::security::ValidationResult::Malicious(reason) => {
                self.security
                    .log_security_event("INVALID_TOOL_NAME", Some(&reason));
        ...
        if args_str.len() > 1024 * 1024 {
            // 1MB limit
            self.security
                .log_security_event("OVERSIZED_TOOL_ARGS", Some(&args_str.len().to_string()));
        ...
        self.security.log_security_event(
            "TOOL_EXECUTION",
            Some(&format!("Server: {}, Tool: {}", server_id, tool_name)),
        );
